...
func TestChannels(t *testing.T) {
	...
	t.Run("no data", func(t *testing.T) {
		t.Run("list", func(t *testing.T) {
			res, err := srvc.List(ctx, &channels.ListPayload{UID: userFoo.Model.FbauthUID})
	...
	t.Run("show", func(t *testing.T) {
		for _, ch := range []*models.Channel{ch1, ch2} {
			t.Run(ch.Name, func(t *testing.T) {
				res, err := srvc.Show(ctx, &channels.ShowPayload{UID: userFoo.Model.FbauthUID, ID: ch.ID})
				assert.NoError(t, err)
				assert.Equal(t, conv.ModelToResult(ch), res)
			})
		}
		t.Run("not found", func(t *testing.T) {
			res, err := srvc.Show(ctx, &channels.ShowPayload{UID: userFoo.Model.FbauthUID, ID: 999})
	...
	t.Run("create", func(t *testing.T) {
		t.Run("valid name", func(t *testing.T) {
			name := "test1"
			res, err := srvc.Create(ctx, &channels.ChannelCreatePayload{UID: userFoo.Model.FbauthUID, Name: name})
			assert.NoError(t, err)
			ch := &models.Channel{ID: res.ID, Name: name, CreatedAt: now, UpdatedAt: now}
			assert.Equal(t, conv.ModelToResult(ch), res)
		})
		t.Run("empty name", func(t *testing.T) {
			res, err := srvc.Create(ctx, &channels.ChannelCreatePayload{UID: userFoo.Model.FbauthUID, Name: ""})
			goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		t.Run("too long name", func(t *testing.T) {
			res, err := srvc.Create(ctx, &channels.ChannelCreatePayload{UID: userFoo.Model.FbauthUID, Name: strings.Repeat("a", 256)})
			goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		...
		t.Run("multi byte characters", func(t *testing.T) {
			...
			t.Run("max", func(t *testing.T) {
				res, err := srvc.Create(ctx, &channels.ChannelCreatePayload{UID: userFoo.Model.FbauthUID, Name: maxMultiByteCharacters})
				assert.NoError(t, err)
				ch := &models.Channel{ID: res.ID, Name: maxMultiByteCharacters, CreatedAt: now, UpdatedAt: now}
				assert.Equal(t, conv.ModelToResult(ch), res)
			})
			t.Run("max plus 1", func(t *testing.T) {
				res, err := srvc.Create(ctx, &channels.ChannelCreatePayload{UID: userFoo.Model.FbauthUID, Name: maxMultiByteCharacters + "a"})
				goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
				assert.Nil(t, res)
			})
		})
	...
	t.Run("update", func(t *testing.T) {
		t.Run("invalid id", func(t *testing.T) {
			res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: 999, Name: "test"})
			goatest.AssertServiceError(t, "not_found", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		t.Run("valid name", func(t *testing.T) {
			...
			defer timetest.SetNow(now)
			name := ch1.Name + "-dash"
			res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID, Name: name})
			assert.NoError(t, err)
			ch := &models.Channel{ID: ch1.ID, Name: name, CreatedAt: before, UpdatedAt: now}
			assert.Equal(t, conv.ModelToResult(ch), res)
		})
		t.Run("empty name", func(t *testing.T) {
			res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID, Name: ""})
			goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		t.Run("too long name", func(t *testing.T) {
			res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID, Name: strings.Repeat("a", 256)})
			goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		...
		t.Run("multi byte characters", func(t *testing.T) {
			maxMultiByteCharacters := strings.Repeat("薔", 255)
			t.Run("max", func(t *testing.T) {
				res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID, Name: maxMultiByteCharacters})
				assert.NoError(t, err)
				ch := &models.Channel{ID: res.ID, Name: maxMultiByteCharacters, CreatedAt: now, UpdatedAt: now}
				assert.Equal(t, conv.ModelToResult(ch), res)
			})
			t.Run("max plus 1", func(t *testing.T) {
				res, err := srvc.Update(ctx, &channels.ChannelUpdatePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID, Name: maxMultiByteCharacters + "a"})
				goatest.AssertServiceError(t, "invalid_payload", err) // ステータスコードは確認できない
				assert.Nil(t, res)
			})
		})
	...
	t.Run("delete", func(t *testing.T) {
		t.Run("invalid id", func(t *testing.T) {
			res, err := srvc.Delete(ctx, &channels.DeletePayload{UID: userFoo.Model.FbauthUID, ID: 999})
			goatest.AssertServiceError(t, "not_found", err) // ステータスコードは確認できない
			assert.Nil(t, res)
		})
		t.Run("valid id", func(t *testing.T) {
			...
			assert.NoError(t, err)
			res, err := srvc.Delete(ctx, &channels.DeletePayload{UID: userFoo.Model.FbauthUID, ID: ch1.ID})
			assert.NoError(t, err)
			assert.Equal(t, conv.ModelToResult(ch), res)
			// DBから削除されていることを確認
