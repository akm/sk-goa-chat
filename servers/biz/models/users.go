...
type User struct {
	...
	FbauthUID string    `boil:"fbauth_uid" json:"fbauth_uid" toml:"fbauth_uid" yaml:"fbauth_uid"`
...
var UserWhere = struct {
	...
	FbauthUID: whereHelperstring{field: "`users`.`fbauth_uid`"},
}
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "name", "email", "fbauth_uid"}
	userColumnsWithoutDefault = []string{"name", "email", "fbauth_uid"}
...
func (userL) LoadChatMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`chat_messages.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
